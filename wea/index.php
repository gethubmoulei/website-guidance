...
header('Access-Control-Allow-Headers:www.yixuewei.com');

//apikey 
$qwkey = "f94b6b17a0c845ed83165ad557ae4a84";
$ipgeokey = "5c2099a74ee646d1be87f7bfd306f6cb";

if (isset($_POST['loca'])){
    $location=htmlspecialchars($_POST["loca"]); 
    //获取城市ID信息
    $cinfo = json_decode(gzdecode(file_get_contents("https://geoapi.qweather.com/v2/city/lookup?location=".$location."&key=".$qwkey)));
    $cid = $cinfo->location[0]->id;

    //获取实时天气
    $wea = json_decode(gzdecode(file_get_contents("https://devapi.qweather.com/v7/weather/now?location=".$cid."&key=".$qwkey)));
    //获取三日天气
    $wea3d = json_decode(gzdecode(file_get_contents("https://devapi.qweather.com/v7/weather/3d?location=".$cid."&key=".$qwkey)));
